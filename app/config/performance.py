...
def ram_mode() -> str:
    ...
    if not _RAM_MODE_LOGGED:
        ...
        raw_value = env_mode_raw if env_mode_raw is not None else "<unset>"
        print(
            f"[PERF] ram_mode_raw={raw_value} ram_mode={_RAM_MODE_CACHE} "
            f"total_ram_gb={total_ram_gb():.1f} "
