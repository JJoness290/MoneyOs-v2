...
class SvdBackend(AiVideoBackend):
    name = "SVD"

    def __init__(self) -> None:
        self.model_id = os.getenv(
            "MONEYOS_SVD_MODEL_ID", "stabilityai/stable-video-diffusion-img2vid-xt"
        )
        self._pipe = None
        self._device = "cpu"
    ...
    def load(self) -> None:
        ...
        if not self.is_available():
            raise BackendUnavailable(
                "SVD model not found in local cache. "
    ...
    def generate(
        ...
        if self._pipe is None:
            raise BackendUnavailable("SVD pipeline not loaded")
