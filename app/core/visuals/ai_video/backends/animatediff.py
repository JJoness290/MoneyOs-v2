...
class AnimateDiffBackend(AiVideoBackend):
    name = "ANIMATEDIFF"

    def __init__(self) -> None:
        self.model_id = os.getenv("MONEYOS_ANIMATEDIFF_MODEL_ID", "guoyww/animatediff")
        self._pipe = None
        self._device = "cpu"
    ...
    def load(self) -> None:
        ...
        if not self.is_available():
            raise BackendUnavailable(
                "AnimateDiff model not found in local cache. "
    ...
    def generate(
        ...
        if self._pipe is None:
            raise BackendUnavailable("AnimateDiff pipeline not loaded")
