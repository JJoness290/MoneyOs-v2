...
from app.core.auto_download import ensure_trueai_video_model, ensure_xtts_model
...
def _script_text(script: dict) -> str:
    ...
    for scene in script.get("scenes", []):
        for beat in scene.get("beats", []):
            parts.append(beat.get("dialogue") or beat.get("on_screen_action") or "")
    return "\n".join(parts)
...
def _tts_for_script(script: dict, text: str, out_dir: Path, spec: EpisodeSpec) -> Path:
    ...
    print(f"[VOICE] using speaker_wav={voice_ref}")

    audio_parts: list[Path] = []
    lines = [line.strip() for line in text.splitlines() if line.strip()]
    for idx, line in enumerate(lines, start=1):
        character = script.get("characters", [{}])[idx % max(1, len(script.get("characters", [])))].get("name", "Narrator")
        voice_id = cast.get(character) or registry.pick_voice(character, "neutral", spec.language)
        # built-in voice first, fallback to speaker_wav
        try:
...
def run_trueai_quality_episode(job_id: str, spec: EpisodeSpec, status_callback=None) -> tuple[Path, Path]:
    ...
    raw_text = _script_text(script)
    clean_text = sanitize_spoken_script(raw_text)
    ...
    run_ffmpeg([
        "ffmpeg", "-y", "-i", str(stitched), "-i", str(audio), "-t", f"{target_seconds:.3f}",
        "-vf", "scale=1920:1080:flags=lanczos,fps=60,format=yuv420p",
        "-c:v", "h264_nvenc", "-preset", "p7", "-cq", "18", "-b:v", "0",
        "-pix_fmt", "yuv420p", "-profile:v", "high", "-g", "120", "-keyint_min", "120",
        "-c:a", "aac", "-ar", "48000", "-b:a", "320k", "-movflags", "+faststart", str(final)
    ])
    report = out_dir / "production_report.json"
    report.write_text(json.dumps({"job_id": job_id, "final": str(final), "minutes": spec.minutes}, indent=2), encoding="utf-8")
    return final, report
